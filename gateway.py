...
class Gateway:
    ...
    def routing(self):
        return redact(dict(self.manager.last_routing))

    def _text(self, payload):
        try:
            requested, messages = validate_chat_request(payload)
        except ValueError as exc:
            raise GatewayError("invalid_request", "Invalid chat request") from exc
        ...
        if requested and not model:
            raise GatewayError("model_not_found", "Requested model is not configured", 404)
    ...
    def chat(self, payload):
        ...
        try:
            ...
            raise GatewayError(f"provider_{exc.kind}", f"Provider routing failed: {exc.kind}", {"auth": 401, "quota": 429, "rate_limit": 429, "timeout": 503, "network": 503, "server": 503}.get(exc.kind, 502)) from exc
        except Exception as exc:
            raise GatewayError("provider_error", "Configured provider failed", 502) from exc
        return chat_response(request_id=f"yasin-{int(time.time() * 1000)}", model=(model or {}).get("name") or requested or "auto", content=str(redact(output)), created=int(time.time()), routing=self.routing())
    ...
    def stream(self, payload) -> Iterator[dict]:
        ...
        try:
            for selected, chunk in self.manager.stream(text, model_name=requested):
                yield {"id": request_id, "object": "chat.completion.chunk", "created": created, "model": selected or model_name, "choices": [{"index": 0, "delta": {"content": redact(chunk)}, "finish_reason": None}]}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send(self, status, data):
        body = json.dumps(redact(data), ensure_ascii=False).encode()
    ...
    def _origin_allowed(self):
        if self.security.origin_allowed(self.headers.get("Origin")):
            return True
        ...
        if payload.get("stream") is True:
            try:
                ...
                for event in self.gateway.stream(payload):
                    self.wfile.write(("data: " + json.dumps(redact(event), ensure_ascii=False) + "\n\n").encode())
                    self.wfile.flush()
                self.wfile.write(b"data: [DONE]\n\n")
                self.wfile.flush()
            except GatewayError as exc:
                try:
                    self.wfile.write(("data: " + json.dumps(error_response(exc.code, exc.message)) + "\n\n").encode())
                    self.wfile.flush()
                except OSError:
                    pass
