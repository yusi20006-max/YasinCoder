...
def safe_path(path, root=None):
    workspace=_workspace(root); candidate=Path(path).expanduser()
    if not candidate.is_absolute(): candidate=workspace/candidate
...
def write_file(path,content,*,apply=False,root=None):
    p=safe_path(path,root); old=p.read_text(encoding="utf-8") if p.exists() else ""
    patch="".join(difflib.unified_diff(old.splitlines(keepends=True),content.splitlines(keepends=True),fromfile=str(p),tofile=str(p)))
    ...
    return result(True,"file.write",applied=True,path=str(p),patch=patch)

def search(pattern,*,root=None,paths:Iterable[str]|None=None):
    w=_workspace(root); cmd=["rg","--line-number","--no-heading",pattern]
    cmd.extend(str(safe_path(p,w)) for p in paths) if paths else cmd.append(str(w))
    try: proc=subprocess.run(cmd,capture_output=True,text=True,timeout=30)
        ...
        for f in w.rglob("*"):
            ...
            try:
                matches += [f"{f}:{n}:{line}" for n,line in enumerate(f.read_text(encoding="utf-8").splitlines(),1) if rx.search(line)]
...
def shell(command,*,root=None,timeout=60):
    p=subprocess.run(command,shell=True,cwd=_workspace(root),capture_output=True,text=True,timeout=timeout)
    return result(p.returncode==0,"shell.exec",command=command,stdout=p.stdout,stderr=p.stderr,returncode=p.returncode)

def git(args:list[str],*,root=None,timeout=60):
    p=subprocess.run(["git",*args],cwd=_workspace(root),capture_output=True,text=True,timeout=timeout)
    return result(p.returncode==0,"git.exec",args=args,stdout=p.stdout,stderr=p.stderr,returncode=p.returncode)
